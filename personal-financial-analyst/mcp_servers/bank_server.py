"""Bank Account MCP Server - Provides simulated bank transaction data."""
...
DATA_FILE: Path = Path(__file__).parent / "mock_data" / "bank_transactions.csv"
...
def _load_transactions_from_csv(
    ...
    logger.info(f"Loading transactions for user: {username}")
    ...
    logger.info(f"Loaded {len(transactions)} transactions for {username}")
    return transactions
...
def get_bank_transactions(
    ...
    """Get bank account transactions for a user within a date range.
    ...
    """
    logger.info(f"[TOOL CALL] get_bank_transactions: username={username}, start_date={start_date}, end_date={end_date}")
    ...
    try:
        ...
        logger.error(f"[TOOL ERROR] Failed to get bank transactions: {e}", exc_info=True)
        raise


if __name__ == "__main__":
    logger.info("=" * 70)
    logger.info("Starting Bank Account MCP Server")
    logger.info("=" * 70)
    ...
    # Run with streamable-http transport on port 5001
    mcp.run(transport="http", port=5001, host="127.0.0.1")
